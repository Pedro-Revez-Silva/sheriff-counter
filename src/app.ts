interface Good {
    ...
    minPlayers?: number;
    countAs?: {
        good: 'apples'| 'cheese' | 'bread' | 'chickens';
        multiplier: number;
    }
}
...
class Game {
    ...
    initializePlayers(count: number) {
        ...
        this.players = Array.from({ length: count }, (_, i) => ({
            id: `player${i + 1}`,
            name: `Player ${i + 1}`,
            scores: availableGoods.reduce((acc, good) => ({...acc, [good.name]: 0}), {}),
            coins: 0
        }));
        this.activePlayerId = 'player1';
        this.updateUI();
    ...
    getEffectiveGoodCount(playerId: string, goodType: string): number {
        const player = this.players.find(p => p.id === playerId);
        if (!player) return 0;
        let count = player.scores[goodType] || 0;
        this.goods
            .filter(good => good.type === 'royal' && good.countAs?.good === goodType)
            .forEach(royalGood => {
                const royalCount = player.scores[royalGood.name] || 0;
                count += royalCount * (royalGood.countAs?.multiplier || 1);
            });
    ...
    getQueenOfItem(item: string): Player | undefined {
        const playersWithItem = this.players.filter(
            player => this.getEffectiveGoodCount(player.id, item) > 0
        );

        if (playersWithItem.length < 2) return undefined;

        const sortedPlayers = playersWithItem.sort(
            (a, b) => this.getEffectiveGoodCount(b.id, item) - this.getEffectiveGoodCount(a.id, item)
        );

        const highestScore = this.getEffectiveGoodCount(sortedPlayers[0].id, item);

        return sortedPlayers.find(
            player => this.getEffectiveGoodCount(player.id, item) < highestScore
        );
    }

    getKingsOfItem(item: string): Player[] {
        const maxScore = Math.max(
            ...this.players.map(p => this.getEffectiveGoodCount(p.id, item))
        );
        return maxScore > 0 ?
